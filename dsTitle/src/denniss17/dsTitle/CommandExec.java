	...
	private boolean cmdTitle(CommandSender sender, Command cmd, String commandlabel, String[] args) {
		...
		if(args[0].equalsIgnoreCase("list")){
			...
			for(Title title: titles){
				if(sender.hasPermission(title.permission)){
					available.add(title.name + " (" + title.title + "&r): " + title.description);
				}else{
					unavailable.add(title.name + " (" + title.title + "&r): " + title.description);
				}
			}
